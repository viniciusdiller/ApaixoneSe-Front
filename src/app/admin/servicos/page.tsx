...
const empty: CreateServicoTuristaDto = { tipo: "GUIA_TURISMO", nome: "", telefone: "", usuarioId: "", instagram: "", descricao: "", endereco: "", cnpj: "", idiomas: "", logoUrl: "", fotoUrl: "" };
...
export default function AdminServicosPage() {
  ...
  const openEdit = (item: ServicoTurista) => {
    setForm({ tipo: item.tipo, nome: item.nome, telefone: item.telefone, usuarioId: item.usuarioId, instagram: item.instagram ?? "", descricao: item.descricao ?? "", endereco: item.endereco ?? "", cnpj: item.cnpj ?? "", roteiro: item.roteiro, idiomas: item.idiomas ?? "", logoUrl: item.logoUrl ?? "", fotoUrl: item.fotoUrl ?? "" });
      ...
      <AdminModal title={modal.editing ? "Editar Serviço" : "Novo Serviço"} open={modal.open} onClose={closeModal}>
        <form onSubmit={handleSave} className="space-y-4">
          <div className="space-y-1">
            ...
            <select value={form.tipo} onChange={set("tipo")} required className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary">
          ...
          <div className="grid grid-cols-2 gap-3">
            <AdminFormField label="URL do Logo" value={form.logoUrl ?? ""} onChange={set("logoUrl")} type="url" />
            <AdminFormField label="URL da Foto" value={form.fotoUrl ?? ""} onChange={set("fotoUrl")} type="url" />
          </div>
          <div className="space-y-1">
            <label className="text-sm font-medium">Usuário Prestador *</label>
            <select value={form.usuarioId} onChange={set("usuarioId")} required className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary">
