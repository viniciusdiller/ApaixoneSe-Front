...
const empty: CreateGastronomiaDto = {
  ...
  documentoPdfUrl: "", logoUrl: "",
  usuarioId: "", instagram: "", especialidade: "",
};
...
export default function AdminGastronomiaPage() {
  ...
  const openEdit = (item: Gastronomia) => {
    setForm({
      ...
      responsavelCpf: item.responsavelCpf, documentoPdfUrl: item.documentoPdfUrl,
      logoUrl: item.logoUrl, usuarioId: item.usuarioId,
      instagram: item.instagram ?? "", especialidade: item.especialidade ?? "",
    });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      modal.editing
        ? await gastronomiaApi.update(modal.editing.id, form)
        : await gastronomiaApi.create(form);
      closeModal(); load();
    } catch (err: unknown) {
  ...
  return (
    <div>
      ...
      {/* Modal Visualização */}
      <AdminModal title="Detalhes do Estabelecimento" open={!!viewing} onClose={() => setViewing(null)}>
        {viewing && (
