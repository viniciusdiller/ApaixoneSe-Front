...
function SingleImageUpload({
  file, existingUrl, onChange, onClear,
}: {
  file: File | null;
      ...
      {previewSrc ? (
        <div className="relative overflow-hidden rounded-xl border border-border">
          ...
          <button type="button" onClick={onClear}
            ...
            <X size={12} />{file ? "Cancelar" : "Trocar imagem"}
          </button>
          {file && (
...
export default function AdminFiquePorDentroPage() {
  ...
  // item que já ocupa a posição selecionada (diferente do que estamos editando)
  ...
  const load = () => {
    setLoading(true);
    fiquePorDentroApi.getAll()
      .then((data) => setItems([...data].sort((a, b) => Number(a.ordem) - Number(b.ordem))))
      .catch(() => setItems([]))
  ...
  function openCreate() { setForm(EMPTY_FORM); setError(""); setModal({ open: true, mode: "create", editing: null }); }
  function openEdit(item: FiquePorDentro) { setForm({ ordem: item.ordem, file: null }); setError(""); setModal({ open: true, mode: "edit", editing: item }); }
    ...
    try {
      if (modal.mode === "create") {
        ...
        if (precisaSwap && swapTarget) {
          // Troca atômica via endpoint dedicado no backend (transação Prisma)
          await fiquePorDentroApi.swap(modal.editing.id, swapTarget.id);

          // Se além do swap também há nova imagem, atualiza em seguida
          if (form.file) {
            const fd = new FormData();
            fd.append("imagem", form.file);
            await fiquePorDentroApi.update(modal.editing.id, fd);
          }

          toast("success", `Posições ${ordemOriginal} e ${ordemNova} trocadas com sucesso!`);
        } else {
          // Posição livre ou só troca de imagem
          const fd = new FormData();
          if (form.file) fd.append("imagem", form.file);
          await fiquePorDentroApi.update(modal.editing.id, fd);
      ...
      if (err instanceof Error) {
        try { const p = JSON.parse(err.message); msg = Array.isArray(p.message) ? p.message.join(", ") : p.message ?? msg; }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="p-2 bg-primary/10 rounded-xl"><GalleryHorizontal className="w-6 h-6 text-primary" /></div>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Fique Por Dentro</h1>
          <p className="text-sm text-muted-foreground">Gerencie as imagens da seção &quot;Fique Por Dentro&quot; da home (máx. 5 imagens).</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={openCreate} disabled={items.length >= 5}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-muted/30 py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">Adicione até 5 imagens para a seção &quot;Fique Por Dentro&quot;.</p>
          <button onClick={openCreate} className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors">
      ...
      {/* Modal */}
      <AdminModal title={modal.mode === "create" ? "Nova Imagem" : "Editar Imagem"} open={modal.open} onClose={closeModal}>
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div>
            ...
            <select value={form.ordem} onChange={(e) => setForm((f) => ({ ...f, ordem: e.target.value }))} required
              ...
              {["1", "2", "3", "4", "5"].map((v) => {
                const ocupante = items.find((it) => it.ordem === v && it.id !== modal.editing?.id);
                const isAtual = modal.editing?.ordem === v;
                return (
                  <option key={v} value={v}>
                    Posição {v}{isAtual ? " (atual)" : ocupante ? " ↔ trocar com esta" : " (livre)"}
                  </option>
                );
              })}
            </select>
