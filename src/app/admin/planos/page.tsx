...
export default function AdminPlanosPage() {
  ...
  const set = (k: keyof CreatePlanoViagemDto) =>
      ...
      {loading ? <LoadingGrid count={3} /> : (
        <AdminTable
          ...
          columns={[
            ...
            { key: "descricao", label: "Descrição", render: (_val, row) => <span className="line-clamp-1 max-w-xs">{row.descricao ?? "—"}</span> },
            { key: "createdAt", label: "Criado em", render: (_val, row) => row.createdAt ? new Date(row.createdAt).toLocaleDateString("pt-BR") : "—" },
          ]}
          extraActions={(row) => (
            <>
