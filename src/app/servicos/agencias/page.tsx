...
export default function AgenciasPage() {
  const [agencias, setAgencias] = useState<ServicoTurista[]>([]);
  ...
  useEffect(() => {
    servicoTuristaApi
      ...
      .then((data) =>
        setAgencias(
          data.filter(
            (s) => s.status === "APROVADO" && s.tipo === "AGENCIA_TURISMO"
          )
        )
      )
      .catch(() => setError(true))
      ...
      <section className="relative overflow-hidden bg-primary px-4 pb-16 pt-32">
        ...
        <div className="container relative z-10 mx-auto">
          ...
          <p className="mt-4 max-w-xl text-lg text-primary-foreground/80">
            Pacotes completos, passeios de barco e roteiros personalizados para
            você curtir Saquarema.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          {!loading && !error && agencias.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center text-muted-foreground">
              ...
              <p className="text-base font-medium">
                Nenhuma agência disponível no momento.
              </p>
              <p className="mt-1 text-sm">
          ...
          {!loading && !error && agencias.length > 0 && (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {agencias.map((a, i) => (
                <ServicoTuristaCard key={a.id} servico={a} index={i} />
              ))}
            </div>
          )}
        </div>
